...
t, avg1, var1 = averaging(totalTime, timeStep, gamma1, 10000)
#t, avg2, var2 = averaging(totalTime, timeStep, gamma2, 1000)
#t, avg3, var3 = averaging(totalTime, timeStep, gamma3, 10000)
...
#plt.semilogy(t, analytical, linestyle="dotted", label="Analytical solution")
#plt.errorbar(t, err)
...
plt.plot(t, avg1, label=r"Simulation, $N = 100, \tau_c = $"+str(t_c1))
plt.fill_between(t, avg1+err1, avg1-err1, alpha=0.5)
#plt.plot(t, analytical1, label=r"Analytical solution, $\tau_c =$"+str(t_c1), linestyle="dashed")
plt.plot(t, tlarge, label=r"Long Time approximation, $\tau_c =$"+str(t_c1), linestyle="dashed")

#plt.plot(t, tlarge, label=r"Analytical solution")

#plt.plot(t, avg2, label=r"Simulation, $N = 100, \tau_c = $"+str(t_c2))
#plt.fill_between(t, avg2+err2, avg2-err2, alpha=0.5)
#plt.plot(t, analytical2, label=r"Analytical solution, $\tau_c =$"+str(t_c2), linestyle="dashed")

#plt.plot(t, avg3, label=r"Simulation, $N = 100, \tau_c = $"+str(t_c3))
#plt.fill_between(t, avg3+err3, avg3-err3, alpha=0.5)
...
plt.ylim([0.8, 0.9])
plt.xlabel(r"Shuttling time $t_s$")
plt.title(r"Long Time Approximation compared with the Simulation")
plt.grid()
plt.legend()
plt.show()