...
avg1, var1 = average_dephasing(totalTime, timeStep, totalLength, xStep, 1, 1, v1, T1, N)
avg2, var2 = average_dephasing(totalTime, timeStep, totalLength, xStep, 1/2, 1/2, v1, T1, N)
avg3, var3 = average_dephasing(totalTime, timeStep, totalLength, xStep, 1/3, 1/3, v1, T1, N)
...
plt.plot(v1, avg3, label=r"$\tau_c, l_c = 3$")
plt.fill_between(v1, avg3+np.sqrt(var3), avg3-np.sqrt(var3), alpha=0.5)


plt.grid()
plt.title("Dephasing coefficient of a single electron spin (2D)")
